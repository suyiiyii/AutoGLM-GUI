...
export interface ChatConversationMessage {
  ...
  /** thinking 是否已完成（用于控制折叠状态） */
  thinkingCompleted?: boolean;
}

interface UseChatAgentConversationResult {
  messages: ChatConversationMessage[];
  setMessages: Dispatch<SetStateAction<ChatConversationMessage[]>>;
  loading: boolean;
  aborting: boolean;
  error: string | null;
...
const CHAT_DEVICE_ID = '__chat__';
const CHAT_DEVICE_SERIAL = 'chat';
const CHAT_SESSION_KEY = 'autoglm:chat-session';
...
function buildAssistantMessage(
  ...
  const isStreaming = isTaskActive(task.status);
  // thinking 已完成 = 任务结束 或 已经开始输出正文
  const thinkingCompleted = !isStreaming || hasSeenContent;
...
export function useChatAgentConversation(): UseChatAgentConversationResult {
      ...
      try {
        ...
        if (storedSessionId) {
          try {
            ...
            if (
              existingSession.device_id !== CHAT_DEVICE_ID ||
              existingSession.device_serial !== CHAT_DEVICE_SERIAL
            ) {
              nextSessionId = null;
            }
          } catch {
        ...
        if (!disposed) {
          console.error('Failed to initialize chat session:', sessionError);
  ...
  const sendMessage = useCallback(
    async (input: string, attachments: TaskImageAttachment[] = []) => {
      ...
      try {
        ...
        console.error('Failed to submit chat task:', sendError);
        setLoading(false);
        setError(
  ...
  const resetConversation = useCallback(async () => {
    ...
    try {
      ...
      sessionStorage.setItem(CHAT_SESSION_KEY, session.id);
