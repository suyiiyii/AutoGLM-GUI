...
export async function removeRemoteDevice(serial: string): Promise<RemoteDeviceRemoveResponse> {
  const res = await axios.post<RemoteDeviceRemoveResponse>('/api/devices/remove_remote', { serial })
  return res.data
}
...
export async function generateQRPairing(timeout: number = 90): Promise<QRPairGenerateResponse> {
  const res = await axios.post<QRPairGenerateResponse>('/api/devices/qr_pair/generate', { timeout })
  return res.data
}
