...
- [ ] `/api/init` 虽 deprecated，但前端和集成测试仍在调用，当前不能直接移除。
...
- [ ] 统一文档与注释，明确 `device_id`、`serial`、`primary_device_id` 的职责边界。
- [ ] 制定 `ConnectionType` 命名收敛方案（避免 `REMOTE` 同时表达“WiFi ADB”和“HTTP Remote Device”语义混淆）。
- [ ] 先做非破坏性整理（注释、映射封装、类型约束），必要时分后续版本做行为级调整。

完成标准：

- [ ] 新增/更新的注释与类型声明能直接说明语义。
- [ ] 新代码不再引入新的命名歧义。

### Phase 4：处理 `/api/init` 的退役路径（延后）

- [ ] 先迁移前端调用（改为自动初始化链路）。
- [ ] 更新集成测试，移除对 `/api/init` 的硬依赖。
- [ ] 完成迁移后再正式删除 `/api/init` 及相关提示文本。

完成标准：

- [ ] `frontend` 不再调用 `/api/init`。
- [ ] 集成测试不再依赖 `/api/init`。
- [ ] API 文档与异常提示同步更新。
