...
当任务失败、设备连接异常或后端行为不符合预期时，日志是定位问题的第一手资料。本指南介绍在不同部署形态下如何查看 AutoGLM-GUI 的运行日志。
...
```
logs/autoglm_{date}.log
```

其中 `{date}` 为当天日期，例如 `logs/autoglm_2026-06-16.log`。同目录下还会有一份只记录错误级别的 `errors_{date}.log`，排查报错时可以优先看它。日志文件会按大小自动轮转，旧文件会被压缩归档。
...
```bash
# 提高日志详细程度，便于排查问题
uv run autoglm-gui --log-level DEBUG

# 指定日志文件路径
uv run autoglm-gui --log-file /var/log/autoglm/app_{time:YYYY-MM-DD}.log

# 关闭文件日志，只输出到控制台
uv run autoglm-gui --no-log-file
```
