...
本文按「问题 → 排查 → 解决」的方式，汇总 AutoGLM-GUI 使用中最常见的故障，覆盖设备连接、模型 API、实时画面、端口冲突和任务执行五类。先按现象找到对应条目，再依次执行排查步骤。
...
- 如果 `adb` 装在非标准路径，用环境变量显式指定：
  ...
  ```bash
  AUTOGLM_ADB_PATH=/path/to/adb autoglm-gui --base-url http://localhost:8080/v1
  ```
...
- 确保模拟器先于 AutoGLM-GUI 启动，或启动后在界面点击「发现设备」刷新。
- 部分模拟器（如夜神、MuMu）需要先执行 `adb connect 127.0.0.1:<端口>` 才能被识别，端口见各模拟器的 ADB 设置。

## 模型 API

模型相关配置遵循四层优先级：CLI 参数 > 环境变量 > 配置文件（`~/.config/autoglm/config.json`）> 默认值。启动横幅会打印当前生效的 `Source`、`Base URL` 和 `Model`，排查时先看这里确认实际用的是哪一份配置。配置方法见 [配置模型](./configure-model.md)。
...
- 想固定端口，用 `--port` 显式指定，例如 `autoglm-gui --base-url ... --port 9000`。
...
这是有意设计的 Fail-Fast 行为，常见于 MCP 的 `chat` 工具和快速执行模式：找不到目标元素时立即报错，而不是猜测坐标乱点。
...
- 日志文件：默认写到 `logs/autoglm_{日期}.log`。需要更详细信息时，启动时加 `--log-level DEBUG`。
